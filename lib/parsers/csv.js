...
export function parseCsvFile(text, { fileName = '', profile: forced } = {}) {
  ...
  if (!dateFormat && txCells.length) {
    ...
    if (d.order) dateFormat = d.order;
      ...
      warnings.push('a coluna de data mistura dd/mm e mm/dd: cada linha foi lida por conta própria e alguma pode estar com dia e mês trocados');
    }
  }
  ...
  if (!decimal && txCells.length) {
    ...
    if (d.decimal) decimal = d.decimal;
      ...
      warnings.push('a coluna de valor mistura vírgula e ponto decimal: cada valor foi lido pelo último separador, e um "1.234" sem casas fica ambíguo');
    }
  }
  ...
  return { profile, transactions: txs, warnings, sep, decimal, dateFormat };
}
