...
function mapHeader(cells, cols) {
  ...
  const idx = {
    ...
    // Coluna de RESERVA para a descrição, usada linha a linha.
    //
    // Não dá para resolver escolhendo "a melhor coluna" no cabeçalho: no
    // extrato da Conta Global do Inter, "Estabelecimento" só é preenchido em
    // compra no cartão — carregamento, chip e resgate vêm vazios. Qualquer que
    // fosse a coluna escolhida, metade do arquivo ficaria sem descrição. E
    // "Sem descrição" não casa com regra nenhuma: cairia tudo em "a revisar"
    // todo mês, para sempre.
    description2: at('description2'),
  };
...
/**
 * Confere a soma contra o saldo inicial e final que o próprio arquivo declara.
 *
 * É diferente da cadeia elo a elo: aqui não importa a ordem das linhas nem se
 * há coluna de saldo por lançamento — basta que
 * `inicial + soma dos lançamentos = final`. O extrato da Conta Global do Inter
 * declara os dois no preâmbulo, e é a única prova interna que ele oferece.
 *
 * Diferente do OFX, AVISA em vez de cancelar, pela mesma razão do resto do
 * leitor de CSV: exportar um recorte de período quebra a conta sem que a
 * leitura esteja errada.
 *
 * @returns {{inicial: number, final: number, somado: number}|null}
 *   null quando o arquivo não declara os dois saldos
 */
/** Identidade que sobrevive a mudança de descrição: data, valor e ordem. */
const idEstavel = (date, cents, ordem) => `${date}|${cents}|${ordem}`;

export function conferirSaldoDeclarado(texto, txs, totals) {
  if (!totals?.opening || !totals?.closing) return null;
  const mi = String(texto).match(totals.opening);
  const mf = String(texto).match(totals.closing);
  if (!mi || !mf) return null;

  const inicial = parseAmountCents(mi[1], ',');
  const final = parseAmountCents(mf[1], ',');
  if (inicial == null || final == null) return null;

  const somado = txs.reduce((s, t) => s + Math.round(t.amount * 100), 0);
  return { inicial, final, somado };
}
...
export function parseCsvFile(text, { fileName = '', profile: forced } = {}) {
  ...
  for (const c of cells) {
    ...
    const description = (idx.description >= 0 && c[idx.description]?.trim())
      ? c[idx.description]
      : (idx.description2 >= 0 && c[idx.description2]?.trim())
      ? c[idx.description2]
    ...
    // Ordinal dentro de (data, valor): dois lançamentos idênticos no mesmo dia
    // são legítimos e precisam continuar sendo dois.
    const chave = `${date}|${cents}`;
    ...
    txs.push({
      ...
      description,
      amount: cents / 100,
      // `stableId` no perfil: identidade derivada de data+valor+ordem, e não da
      // descrição. Existe porque o MESMO extrato pode chegar em dois formatos —
      // a Conta Global do Inter exporta PDF e CSV — e as descrições diferem
      // ("MERPAGO*ALTOSPORT" no CSV, "MERPAGOALTOSPORT" no PDF). Com hash de
      // conteúdo, importar os dois dobraria tudo, calado.
      externalId: (idx.id >= 0 && c[idx.id]) ? c[idx.id]
        : spec.stableId ? idEstavel(date, cents, ordem) : null,
      source: profile?.source || 'csv',
      transfer: false,
      balance: saldo == null ? null : saldo / 100,
      // Moeda declarada pelo PERFIL, não pela instalação. Nulo = "a da
      // instalação" e é o caso normal; só extrato em moeda estrangeira
      // preenche. Ver o comentário de transactions.currency em lib/db.js.
      currency: profile?.currency ?? null,
    });
  }

  const declarado = conferirSaldoDeclarado(raw, txs, spec.totals);
  if (declarado && Math.abs(declarado.inicial + declarado.somado - declarado.final) > 1) {
    warnings.push(t('import.warn.csvOpeningClosing', {
      opening: (declarado.inicial / 100).toFixed(2),
      closing: (declarado.final / 100).toFixed(2),
      sum: (declarado.somado / 100).toFixed(2),
      expected: ((declarado.final - declarado.inicial) / 100).toFixed(2),
    }));
  }
  ...
  return {
    profile,
    transactions: txs.map(({ balance, ...t }) => t),   // saldo era só para conferir
