...
export function computeInsights({ transactions, goals = [], cards = [], billOccurrences = [], now = new Date() }) {
  const insights = [];
  ...
  for (const o of billOccurrences) {
    if (o.status === 'atrasada') {
      insights.push({
        ...
        kind: 'conta_atrasada',
        severity: 'alerta',
        title: `Conta atrasada: ${o.description}`,
        detail: `Venceu em ${o.due_date.slice(8, 10)}/${o.due_date.slice(5, 7)} · ${fmtBRL(o.amount_cents)}`,
      ...
      if (days <= 3) {
        insights.push({
          ...
          kind: 'conta_proxima',
          severity: 'atencao',
          title: `${o.description} vence ${days === 0 ? 'hoje' : days === 1 ? 'amanhã' : `em ${days} dias`}`,
          detail: fmtBRL(o.amount_cents),
          cents: o.amount_cents,
        });
      }
    }
  ...
  if (histMonths.length >= 2) {
    ...
    if (best) {
      insights.push({
        ...
        cents: best.cut,
        action: { label: 'Simular investindo isso', href: `/evoluir?aporte=${best.cut}` },
      });
    }
  }
