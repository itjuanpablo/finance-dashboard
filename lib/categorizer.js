...
const KEYWORDS_AR = [
  ...
  { kw: ['impuesto al debito', 'impuesto al credito', 'ley 25413', 'percepcion', 'retencion', 'iva ', 'iibb', 'ingresos brutos', 'sellado', 'interes', 'intereses', 'comision', 'mantenimiento de cuenta', 'seguro', 'prestamo'], cat: CAT.FINANCIAL },
  ...
  // Renda
  { kw: ['sueldo', 'haberes', 'honorarios', 'transferencia recibida', 'reintegro', 'devolucion', 'plazo fijo', 'rendimientos'], cat: CAT.INCOME },
];

/** Dicionários por região, expostos para teste (a instância usa um só). */
export const KEYWORD_DICTS = { br: KEYWORDS_BR, ar: KEYWORDS_AR };

/** Dicionário desta instância, escolhido por REGION.keywords. */
export const ACTIVE_KEYWORDS = KEYWORD_DICTS[REGION.keywords] || KEYWORDS_BR;
 ...
 * @param {Array<{kw: Array<string|RegExp>, cat: string}>} [dict] dicionário (padrão: o da região)
 * @returns {string} chave de categoria
 */
export function categorize(description, rules = [], dict = ACTIVE_KEYWORDS) {
