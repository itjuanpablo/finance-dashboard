...
import { detectNubankExtrato } from '../parsers/nubank-extrato.js';
import { detectInterGlobalPdf } from '../parsers/inter-global-pdf.js';
...
export const DEFAULT_COLUMNS = {
  ...
  // Reserva da descrição — ver `description2` em lib/parsers/csv.js
  description2: { any: ['tipo da transacao', 'tipo de transaccion'] },
...
const BR_PROFILES = [
  ...
  {
    id: 'nubank-extrato-pdf',
    name: 'Nubank',
    kind: 'statement',
    country: 'BR',
    formats: ['pdf'],
    source: 'nubank-extrato',
    // Confiança: ALTA — calibrado contra extrato real. O subtotal de cada dia é
    // conferido ao centavo, e divergência cancela a importação: aqui o sinal de
    // cada linha é inferido do agrupamento, então a soma é a única prova.
    confidence: 'alta',
    detect: (text) => detectNubankExtrato(text),
  },
  {
    id: 'inter-global-pdf',
    name: 'Inter Global',
    ...
    source: 'inter-global',
    currency: 'USD',
    // Confiança: ALTA — o sinal vem do tipo da transação e a cadeia de saldos
    // diários confere, fechando até o saldo inicial declarado no topo.
    confidence: 'alta',
    detect: (text) => detectInterGlobalPdf(text),
  },
  {
    // ─── Conta Global do Inter: o extrato inteiro é em DÓLAR ────────────────
    // Não é uma conta em reais com compras internacionais: é uma conta corrente
    // em USD nas Ilhas Cayman, com saldo em USD. Por isso o perfil declara
    // `currency: 'USD'` — e essa declaração é o que impede o app de somar 86,69
    // dólares com reais e mostrar um total que nunca existiu.
    //
    // Vem ANTES do `inter-csv` (extrato em reais) porque os dois são do mesmo
    // banco e o genérico casaria primeiro. Os marcadores aqui são do documento
    // em dólar, não do Inter em geral.
    //
    // O PDF equivalente NÃO tem perfil, e é decisão: naquele arquivo o sinal do
    // lançamento não existe — "Beneficiário" e "Remetente" aparecem tanto em
    // débito quanto em crédito, e a direção só sai reconstruindo a cadeia de
    // "Saldo do dia". O CSV do mesmo banco traz Débito/Crédito escrito. Ler o
    // arquivo pior quando o melhor está disponível é criar risco à toa.
    id: 'inter-global-csv',
    name: 'Inter Global',
    kind: 'statement',
    country: 'BR',
    ...
    source: 'inter-global',
    currency: 'USD',
    // Confiança: ALTA — calibrado contra um extrato real de 8 movimentos. O
    // cabeçalho declara saldo inicial e final, e a soma é conferida contra os
    // dois a cada importação.
    confidence: 'alta',
    csv: {
      sep: ';',
      dateFormat: 'dmy',
      decimal: ',',
      // O valor vem sempre positivo ("US$ 9,38"); quem dá o sinal é a coluna
      // "Tipo da Operação" (Débito/Crédito).
      amountSign: 'auto',
      // O mesmo extrato existe em PDF; a identidade tem de casar entre os dois.
      stableId: true,
      columns: {
        date: ['data da transacao'],
        amount: ['valor da transacao'],
        // "Estabelecimento" só existe em compra no cartão; carregamento e chip
        // vêm vazios. Sem o segundo nome, metade do extrato virava
        // "Sem descrição" — e "Sem descrição" não casa com regra nenhuma, então
        // cairia tudo em "a revisar" todo mês.
        description: ['estabelecimento'],
        description2: ['tipo da transacao'],
        type: ['tipo da operacao'],
      },
      // Saldo inicial e final do período, declarados no preâmbulo. É com eles
      // que a leitura é conferida (ver assertSaldoGlobal em lib/parsers/csv.js).
      totals: {
        opening: /Saldo Inicial Per[ií]odo Solicitado;\s*\$?\s*(-?[\d.]+,\d{2})/i,
        closing: /Saldo Final Per[ií]odo Solicitado;\s*\$?\s*(-?[\d.]+,\d{2})/i,
      },
    },
    detect: (text) =>
      /Extrato Global Account/i.test(text) ||
      (/Global Account/i.test(text) && /Tipo da Opera[çc][ãa]o/i.test(text)),
  },
  {
    id: 'inter-csv',
    name: 'Inter',
