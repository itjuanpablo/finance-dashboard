...
const BR_PROFILES = [
  ...
  },
  {
    ...
    detect: (text) => lineWithAll(text, ['data', 'historico', 'credito', 'debito']),
  ...
  },
  {
    ...
    detect: (text) => lineWith(text, ['data', 'historico'], ['balancete', 'dependencia']),
...
const AR_PROFILES = [
  ...
  },
  {
    ...
    formats: ['pdf'],
    source: 'mp-ar-fatura',
    // Confiança: BAIXA — NÃO VALIDADO. Ver lib/parsers/mercadopago.js.
    confidence: 'baixa',
    detect: (text) => detectMercadoPago(text).id === 'mp-ar-resumen',
  },
  {
    id: 'galicia-csv',
    name: 'Banco Galicia',
